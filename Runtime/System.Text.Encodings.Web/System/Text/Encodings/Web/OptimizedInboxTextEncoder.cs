...
namespace System.Text.Encodings.Web
{
    ...
    internal sealed partial class OptimizedInboxTextEncoder
    {
...
#if DEBUG && !NETCOREAPP3_1 && !UNITY_2020
        ...
        public unsafe int GetIndexOfFirstCharToEncode(ReadOnlySpan<char> data)
        {
            ...
            {
                uint lengthInChars = (uint)data.Length;
                ...
                if (idx < lengthInChars)
                {
                    ...
                    {
                        loopIter = -1;
                        if (!_allowedBmpCodePoints.IsCharAllowed(pData[idx + (uint)(++loopIter)])) { goto BrokeInUnrolledLoop; }
                        if (!_allowedBmpCodePoints.IsCharAllowed(pData[idx + (uint)(++loopIter)])) { goto BrokeInUnrolledLoop; }
                        if (!_allowedBmpCodePoints.IsCharAllowed(pData[idx + (uint)(++loopIter)])) { goto BrokeInUnrolledLoop; }
                        if (!_allowedBmpCodePoints.IsCharAllowed(pData[idx + (uint)(++loopIter)])) { goto BrokeInUnrolledLoop; }
                        if (!_allowedBmpCodePoints.IsCharAllowed(pData[idx + (uint)(++loopIter)])) { goto BrokeInUnrolledLoop; }
